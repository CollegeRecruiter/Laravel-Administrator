...
class Factory {
	...
	{
		//make sure we only run this once and then return the cached version
		if (!($this->columnOptions && sizeof($this->columnOptions)))
		{
	...
	{
		//make sure we only run this once and then return the cached version
		if (!($this->includedColumns && sizeof($this->includedColumns)))
		{
			$model = $this->config->getDataModel();
	...
	{
		//make sure we only run this once and then return the cached version
		if (!($this->relatedColumns && sizeof($this->relatedColumns)))
		{
			foreach ($this->getColumns() as $column)
			{
				if ($column->getOption('is_related'))
	...
	{
		//make sure we only run this once and then return the cached version
		if (!($this->computedColumns && sizeof($this->computedColumns)))
		{
			foreach ($this->getColumns() as $column)
			{
